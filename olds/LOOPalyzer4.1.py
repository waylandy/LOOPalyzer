...
def main():
    if len(sys.argv) != 2:
        ...
        print '##############################################################'
        print ''
        print 'LOOPalyzer version 4.1'
        print ''
        print 'USAGE: python LOOPalyzer4.1.py sequences.fastq'
        print ''
        print 'This program will only accept one argument: a fastq sequence'
        print 'LOOPalyzer checks fastq sequencing files from LOOPER-generated'
        print 'libraries for mutations and then extracts features and outputs'
        print 'them to a csv file.'
        print ''
        print 'Parameters such as the primers sequences and reading region'
        print 'can be changing within this Python script. All parameters are'
        print 'located at the top of the script. Please note that changing'
        print 'parameters other than the primer sequences may make attribute'
        print 'labels (also listed within the script) obselete. Please make'
        print 'sure to keep track of these yourself.'
        print ''
        print '##############################################################'
        print ''
    else:
        ...
        print '##############################################################'
        print ''
        print 'LOOPalyzer version 4.1 now running!'
        print ''
        print 'Sequences will be checked for mutations'
        print 'Features are extracted from non-mutated sequences'
        print ''
        start = time.time()
        # loop that iterates through the FASTQ file
        with open(file + '_ATTR.csv', 'w') as w:
            n = 0
            with open(file) as f:
                for l in f:
                    n += 1
        ...
        with open(file) as f:
            ...
            c = i+1
        # results summary
        print str(int(c/4)) + ' sequences processed'
        print 'Elapsed time: ' + str(round(time.time() - start, 2)) + ' seconds'
        print 'Output saved as: ' + file + '_ATTR.csv'
        print ''
        print 'The output file can be analyzed with Microsoft Excel or with'
        print 'alternative methods such as the pandas module in Python'
        print ''
        print '##############################################################'
        print ''
